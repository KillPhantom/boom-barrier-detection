...
# if __name__ =='__main__':
	...
	image=cv2.imread('night2/%d.jpg' %i)
	...
	blur = gaussian_blur(gray, 3)#高斯模糊
	edge = canny(blur, 50, 100)# 边缘检测
	mask = region_of_interest(edge, v)#取roi
	lines = cv2.HoughLinesP(mask, 0.8, np.pi/180, 25,minLineLength=30,maxLineGap=6)#统计霍夫变换
	...
	print(i)
	# 下面是判断条件
	if lines is None : #你打开张图看看就知道了 
		
		print('杆未抬起')
		img=cv2.putText(image,'fallen',(40,100),font,2.5,(0,0,255),5) #写字，参数百度
		cv2.imwrite('night2_result/result%d.jpg' %i,img)
